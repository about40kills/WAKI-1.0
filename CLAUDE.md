...
3. **LLM** — `claudeClient.ts` sends last 10 turns to Claude (`claude-sonnet-4-20250514`), receiving a JSON response: `{ response, risk_level, risk_indicators }`
...
### Service Layer
- `src/services/llm/claudeClient.ts` — Claude API calls, retry on 529 (up to 2x with backoff)
...
- `src/services/location/nearbyFacilities.ts` — Uses `expo-location` + OpenStreetMap Overpass API to find hospitals, clinics, and mental health facilities within 10 km of the user's device location
- `src/services/location/phoneLookup.ts` — Best-effort web scraping (DuckDuckGo) to find Ghana phone numbers for facilities missing contact info from OSM; extracts and normalises `+233` patterns
...
- Claude always responds as JSON: `{ response, risk_level, risk_indicators }`
...
- Crisis contacts (Ghana emergency numbers) are embedded in `src/constants/crisisContacts.ts` — never fetched at runtime; numbers were revised in April 2026 against official / directory sources (see `docs/CRISIS_PROTOCOL.md` and the file header in `crisisContacts.ts`)
- The crisis screen also offers a "Find hospitals near me" feature using device geolocation + OpenStreetMap. Phone numbers sourced via web scraping are best-effort and **not clinically verified** — consider replacing with Google Places API or a curated list for production use
- Tapping a nearby facility opens Apple Maps / the default maps handler using **`maps://maps.apple.com/?ll=lat,lon`** on iOS (coordinates from Overpass). Do not use **`maps:0,0`** — that literal centre is Null Island (Gulf of Guinea)
